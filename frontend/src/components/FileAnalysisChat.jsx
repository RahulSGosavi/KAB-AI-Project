...
const FileAnalysisChat = ({ isOpen, onClose, projectFiles = [] }) => {
  ...
  const parseExcelFile = useCallback(async (arrayBuffer, filename) => {
    // ... (Excel parsing logic remains the same: finds header row, extracts cells and rawHeaders) ...
    ...
    for (let idx = 0; idx < sheetsToProcess.length; idx++) {
      ...
      const headers = rawHeaders.map(h => {
        try {
          ...
          // Guarantee 'h' is a string before calling replace, trim, toLowerCase.
          const safeH = String(h || '');
          
          return safeH.replace(/[^a-zA-Z0-9\s]/g, ' ').replace(/\s+/g, ' ').trim().toLowerCase();
        } catch {
          return '';
        }
      });
  ...
  const findPricesForSKU = useCallback((sku) => {
    // ... (SKU finding logic remains the same, but uses the improved extractMaterialName) ...
          ...
          <div className="p-5 border-t bg-white">
            <div className="flex gap-3">
              ...
              <button
                ...
                className="px-6 py-3 bg-indigo-600 text-white rounded-lg font-medium hover:-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-2"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
